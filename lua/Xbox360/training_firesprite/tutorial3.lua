-- MULTIPLAYER FIX DATE: May 18, 2024 (XBOX 360)
--NOTES: UNTESTED!
complete = 0
taskcount = 4
currenttask = 1
...
function success()
  ...
  if #successText > 0 then
    gomgr.getbyoid(2053).finishonbuttonpress = false
    for i = 1, #successText do
      gomgr.getbyoid(2053).location = 0
      gomgr.getbyoid(2053).portrait = 1
      gomgr.getbyoid(2053):settext(" textbox ", successText[i])
      if i == #successText then gomgr.getbyoid(2053).finishonbuttonpress = true end
      beginwait()
        gomgr.getbyoid(2053):start()
      endwait()
    end
    gomgr.getbyoid(2053):finish()
  end
  
  gomgr.getbyoid(2054):finish()
...
function reset(text)
  ...
  --           
  gomgr.getbyoid(2053).location = 1
  gomgr.getbyoid(2053).buttonon = true
  gomgr.getbyoid(2053).finishonbuttonpress = false
  for i = 1, #text do
    gomgr.getbyoid(2053):settext(" textbox ", text[i])
    if i == #text then gomgr.getbyoid(2053).finishonbuttonpress = true end
    beginwait()
      gomgr.getbyoid(2053):start()
    endwait()
  end
  ...
  --               
  gomgr.getbyoid(2054):cleartasktext()
  gomgr.getbyoid(2054).taskcount = #taskText
  for i = 1, #taskText do
    gomgr.getbyoid(2054):settasktext(i-1, taskText[i])
  end
  gomgr.getbyoid(2054):start()
...
function on_gameplaymoduleupdate()
  ...
  if (taskSuccess[currenttask]()) then
    gomgr.getbyoid(2054):settaskcomplete(currenttask-1)
  ...
  if (taskFail[currenttask]()) then
    complete = -1
    gomgr.getbyoid(2054):settaskfail(currenttask-1)
    pause(2.5)
    ...
    if (failText[currenttask]) then
      ...
      gomgr.getbyoid(2053).location = 0
      gomgr.getbyoid(2053).portrait = 1
      gomgr.getbyoid(2053):settext(" textbox ", failText[currenttask])
      beginwait()
        gomgr.getbyoid(2053):start()
      endwait()
      gomgr.getbyoid(2053):finish()
